...
class OvercookedGridworld(object):
    ...
    def set_layout_params(self, layout_name):
        ...
        for name in all_layout_params:
            if name != "random0":
                all_layout_params[name]['shared_counters'] = list(
                    all_layout_params[name]['counter_location_to_id'].keys())
            ...
            if terrain_type == 'X':
                ...
                if player.has_object() and not new_state.has_object(i_pos):
                    ...
                    try:
                        ...
                        if self.item_tracking_dict[picked_item_uid]['n_touches'] > len(
                                np.unique(self.item_tracking_dict[picked_item_uid]['past_players'])):
    ...
    def get_high_level_interact_action(self, new_state, joint_action, n_features=7):
        ...
        ACTION_DIRECTION_TO_HIGH_LEVEL = {Direction.NORTH: NORTH, Direction.SOUTH: SOUTH, Direction.EAST: EAST,
                                          Direction.WEST: WEST}
        ALL_HIGH_LEVEL_ACTIONS = [NORTH, SOUTH, EAST, WEST, STAY, PICKUP_ONION_FROM_DISPENSER,
                                  PICKUP_DISH_FROM_DISPENSER,
                                  PUT_DOWN_ONION_ON_COUNTER, PUT_DOWN_ONION_IN_POT, PUT_DOWN_DISH_ON_COUNTER,
                                  PUT_DOWN_SOUP_ON_COUNTER,
    ...
    def resolve_movement(self, state, joint_action):
        ...
        new_positions, new_orientations, collided = self.compute_new_positions_and_orientations(state.players,
                                                                                                joint_action)
